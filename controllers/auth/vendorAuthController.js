...
exports.signup = async (req, res) => {
  try {
    ...
    console.info("✅ Vendor populated with university items");
    ...
    console.info("✅ Vendor added to Uni's vendors array");
...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isVerified) {
      // Generate new OTP
      const otp = generateOtp();
      console.info("🔢 Generated OTP for login:", otp, "for email:", user.email);
      
      // Delete any existing OTPs for this email first
      await Otp.deleteMany({ email: user.email });
      console.info("🗑️ Deleted existing OTPs for email:", user.email);
      
      // Save new OTP with lowercase email
      const newOtp = new Otp({ email: user.email.toLowerCase(), otp, createdAt: Date.now() });
      await newOtp.save();
      console.info("✅ Saved new OTP:", newOtp);

      // Send OTP email
      await sendOtpEmail(user.email, otp);
      console.info("📧 Sent OTP email to:", user.email);
