...
exports.getCart = async (req, res) => {
  try {
    ...
    // BOLA Check
    if (!validateUserAccess(req, userId)) {
      return res.status(403).json({ message: "Unauthorized: You can only view your own cart." });
    }
...
exports.getExtras = async (req, res) => {
  try {
    ...
    // BOLA Check
    if (!validateUserAccess(req, userId)) {
      return res.status(403).json({ message: "Unauthorized: You can only view your own cart extras." });
    }
